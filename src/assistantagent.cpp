...
QString AssistantAgent::buildSystemPrompt() const
{
    ...
    lines << QStringLiteral(
        ...
        "- Python editor: model script (load_model / get_model_python / replace_in_model_python)\n"
        "- Results / calculator panels\n"
        "\n"
        "Typical workflows (use tools, do not narrate steps for the user):\n"
        "1) load_model from examples/… → get_model_python → replace_in_model_python for "
        "script parameters (fstop, mesh, paths) → save_model → run_simulation.\n"
        "2) Geometry size tweaks on a fixed GDS: call modify_gds with "
        "current_diameter_um + diameter_delta_um (or scale). It creates .gds.bak once, "
        "runs KLayout batch, reloads layout. This is uniform XY scale about the cell "
        "centre — not a full PDK PCell re-parameterization.\n"
        "3) eval_rf for Results calculator after a sim.\n"
        "\n"
        "Rules:\n"
